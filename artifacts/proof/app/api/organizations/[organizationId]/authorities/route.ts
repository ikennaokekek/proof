...
export async function POST(request: Request, { params }: { params: Promise<{ organizationId: string }> }) {
  ...
  const r = await supabaseRequest("/rest/v1/rpc/grant_approval_authority", { method: "POST", body: JSON.stringify({ target_org: organizationId, target_member: parsed.data.memberId, authority_category: parsed.data.category }) }, session.accessToken);
  ...
  logSecurityEvent("authority.grant", request, "succeeded", { organizationId, memberId: parsed.data.memberId, category: parsed.data.category });
  return Response.json(await r.json(), { status: 201 });
}